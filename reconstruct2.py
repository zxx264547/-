...
# 生成输入输出样本
def split_sequence_test(seq, n_steps_in, label):
    X, y = list(), list()
    loc = list()
    for i in range(len(seq)):
        ...
        if end_ix > len(seq):
            break
        seq_x, seq_y = seq[ii:end_ix, 0:-1], seq[end_ix-1, -1]
        seq_x = np.array(seq_x)
        seq_x = np.c_[seq_x, seq[ii - 1:end_ix - 1, -1]]
        seq_x = np.array(seq_x)
        seq_x = seq_x.flatten()
        if (label[end_ix-3] == 0) & (label[end_ix-2] == 0) & (label[end_ix-1] == 1):
            X.append(seq_x)
            y.append(seq_y)
            loc.append(end_ix-1)
...
# 生成输入输出样本
def split_sequence_train(seq, n_steps_in, label):
    X, y = list(), list()
    loc = list()
    for i in range(len(seq)):
        ii = i + 1
        ...
        seq_x = np.array(seq_x)
        seq_x = seq_x.flatten()
        if (label[end_ix-3] == 0) & (label[end_ix-2] == 0) & (label[end_ix-1] == 0):
            X.append(seq_x)
            y.append(seq_y)
            loc.append(end_ix-1)
    return np.array(X), np.ravel(np.array(y)), np.array(loc)
...
X_test2, y_test2, loc_test = split_sequence_test(data[:, [0, 1, 2, 3]], 2, label)
X_train2, y_train2, loc_train = split_sequence_train(data[:, [0, 1, 2, 3]], 2, label)
# 归一化处理
