...
if __name__ == "__main__":
    ...
    # 保存结果
    score = model.cv_results_['mean_test_score']
    score = score.reshape((-1, 3), order='F')
    results = {'n_neighbors': k_list, 'largest': score[:, 0], 'mean': score[:, 1], 'median': score[:, 2]}
    df = pd.DataFrame(results)
    df.to_excel('tune_knn.xlsx', engine='xlsxwriter')
