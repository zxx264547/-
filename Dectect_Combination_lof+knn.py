...
if __name__ == "__main__":
    ...
    n_clf = 40  # number of base detectors
    ...
    #     clf.fit(X_train_norm)
    #     train_scores[:, i+n_clf//2] = clf.decision_scores_
    ...
    plt.savefig('roc曲线_MOA_20knn')
    plt.show()

    # 识别结果可视化
    label_moa = score_to_label(y_by_moa, outliers_fraction=rate)
    plt.scatter(X_train[:, 0], X_train[:, 1], marker='o', c=label_moa, cmap='rainbow', s=10)
    plt.savefig('MOA_20knn')
    plt.show()
